...
export default function UserDashboard() {
  ...
  // NEW: Function to validate date
  const validateDate = (dateString) => {
    if (!dateString) return { isValid: true, message: "" }; // Empty date is allowed
    
    const selectedDate = new Date(dateString);
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Remove time part for accurate comparison
    
    // Reset time part of selected date for comparison
  ...
  const startEdit = (todo) => {
    ...
    setDueDate(todo.dueDate || "");
    setPriority(todo.priority || "medium");
    setDateError(""); // Clear date error when starting edit
  };
  ...
  const handleDateChange = (e) => {
    ...
    // Validate date in real-time (optional)
    if (value) {
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1">
                <input
                  ...
                  min={getTodayDate()} // ✅ Disable past dates in calendar
                  className={`px-4 py-3 w-full rounded-xl bg-white/80 backdrop-blur-sm border ${
                    dateError ? "border-red-300 focus:ring-red-500/20" : "border-purple-200 focus:ring-purple-500/20"
                  } focus:bg-white focus:ring-4 focus:border-purple-500 outline-none transition-all disabled:opacity-60`}
                  disabled={isAdding || isUpdating}
                />
                {dateError && (
                  <p className="text-red-500 text-sm mt-1 px-1">{dateError}</p>
                )}
              </div>
                          ...
                          <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                            {todo.dueDate && (
                              <span className="flex items-center gap-1">
                                <CalendarDays size={12} />
                                {new Date(todo.dueDate).toLocaleDateString("en-IN")}
                                {/* Show warning if past due date */}
                                {new Date(todo.dueDate) < new Date() && !todo.completed && (
                                  <span className="text-red-500 text-xs font-bold">(Overdue)</span>
                                )}
                              </span>
                            )}
